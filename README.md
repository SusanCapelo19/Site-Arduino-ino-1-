...
(Isso é equivalente a somar/subtrair um constante à função, ou seja, deslocar a onda verticalmente.)
 

Isso é especialmente útil, pois o conversor ADC (analógico->digital) do Arduino lê apenas sinais de 0V a 5V.

Assim, se as ondas do microfone estivessem centradas em 0V, todos os sinais abaixo de 0V não seriam lidos corretamente pelo Arduino.

Recebendo o sinal analógico, o Arduino converte a voltagem para um valor digital entre 0 e 1023 (1 byte).

Caso não houvesse o módulo, poderia-se polarizar a saída usando um divisor de tensão. Segundo a fórmula, voltagem de saída = (R2÷(R1+R2))*voltagem de entrada.
...
Além disso, deve-se considerar que, por padrão, o Arduino usa uma referência de voltagem de 5V para o ADC. Ou seja, uma entrada de 5V retorna 1023 e uma entrada de 0V retorna 0.
...
o	0 = 0b00000000 (nenhuma linha)
o	1 = 0b00000001 (primeira linha)
o	3 = 0b00000011 (primeira e segunda linha)
o	…
o	255 = 0b1111111 (todas as linhas)
...
Em seguida, o loop:
1.	Atualiza a nova medição (que indica qual foi a última vez em que o loop fez uma amostra)
2.	Lê o sinal analógico de A0
3.	Retira a polarização da voltagem do microfone (se lembra de que o microfone retorna um valor próxima de 512, mesmo sem som?)
4.	Filtra os sinais graves da amostra
5.	Salva o sinal filtrado no vetor de valores reais (e salva 0 no vetor de valores imaginários)
...
a.	Usamos números menores ao trabalhar com floats (gastamos menos memória).
b.	O FFT não precisa de voltagens absolutas, já que calcula o conteúdo de frequências de forma relativa. Se dividirmos todas as amostras por um mesmo valor, o FFT ainda funciona.
c.	Quanto maior os valores da voltagem, mais responsivo é o FFT (pode retornar mais ruídos).
...
grupo frequência=taxa de amostragem número de amostras=1000064=156.25Hz
